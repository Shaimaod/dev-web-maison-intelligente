                            ...
                            @if($object->category === 'Climatisation')
                                <div class="temperature-control">
                                    <div class="temperature-display">
                                        <span id="currentTempDisplay">{{ $object->current_temp ?? 20 }}</span>
                                    </div>
                                    <div class="temperature-range">
                                        <input type="range" class="form-range" id="temperatureInput" min="16" max="30" value="{{ $object->target_temp ?? 20 }}" step="0.5">
                                        <div class="temperature-labels">
                                            <span>16°C</span>
                                            <span id="targetTempDisplay">{{ $object->target_temp ?? 20 }}</span>
                                            <span>30°C</span>
                                        </div>
                                    </div>
...
function updateObject() {
    ...
    @if($object->category === 'Climatisation')
        data.target_temp = document.getElementById('temperatureInput').value;
    @endif
    ...
    .then(data => {
        ...
        @if($object->category === 'Climatisation')
            if (data.object.current_temp) {
                document.getElementById('current_temp').textContent = data.object.current_temp + '°C';
                document.getElementById('currentTempDisplay').textContent = data.object.current_temp;
            }
            if (data.object.target_temp) {
                document.getElementById('target_temp').textContent = data.object.target_temp + '°C';
                document.getElementById('targetTempDisplay').textContent = data.object.target_temp;
            }
        @endif
...
function requestDeletion() {
    ...
    fetch('/object/{{ $object->id }}/request-deletion', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
    ...
    .then(data => {
        ...
        // Rediriger vers la liste des objets connectés après un court délai
        setTimeout(() => {
            window.location.href = '{{ route("connected.objects") }}';
        }, 2000);
    })
    .catch(error => {
        console.error('Error:', error);
